<template>
  <div class="p-6 flex flex-col gap-5 font-poppins">
    <div v-if="props.searchable" class="w-full flex flex-col">
      <InputGroup class="py-7 border-2 bg-gray-50">
        <InputGroupInput
          type="search"
    ...
    <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400">
      <thead
        class="text-xs text-gray-700 dark:text-gray-300 uppercase bg-gray-50 py-7"
      >
        <tr>
          <th
          ...
          <th v-if="slots.actions" class="px-6 py-4 font-semibold" scope="col">
        ...
        >
          ...
          >
            <slot
              ...
              :item="item"
              :value="getCellValue(item, header)"
            >
              <span
                v-if="header.type === 'status' && header.statusConfig"
                :class="
                  getStatusBadgeClass(getStatusConfig(item, header).variant)
                "
              >
      ...
      <ul class="inline-flex items-stretch -space-x-px">
        <li>
          <a
            ...
            :class="[
              'flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-background-dark dark:border-white/20 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-white',
        ...
        <li v-for="pageNum in displayedPages" :key="pageNum">
          <a
            ...
            :class="[
              'flex items-center justify-center px-3 py-3 text-sm leading-tight border',
              currentPage === pageNum
        ...
        <li>
          <a
            ...
            :class="[
              'flex items-center justify-center h-full py-1.5 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-background-dark dark:border-white/20 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-white',
